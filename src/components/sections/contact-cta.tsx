...
/** Abre WhatsApp; se o popup for bloqueado, navega na mesma aba. */
function openWhatsApp(url: string) {
  const popup = window.open(url, '_blank', 'noopener,noreferrer')
  if (popup) return 'popup' as const
...
export function ContactCta() {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    // Validação síncrona no gesto do clique — evita popup blocker da Vercel
    // (zodResolver/RHF é async e quebra a cadeia do user-gesture).
    const parsed = contactFormSchema.safeParse(getValues())
